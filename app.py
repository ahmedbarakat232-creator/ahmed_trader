...
def load_saved_settings():
    if os.path.exists(DB_FILE):
        try:
            ...
            pass
    # تعديل 1: الأسهم الافتراضية الجديدة
    return {
...
with st.expander("⚙️ إعدادات المحفظة والاتصال والإشعارات"):
    watchlist_input = st.text_area("أدخل الرموز لقائمة مراقبتك (مثل: NVDA,TSLA,GC=F):", value=settings.get("watchlist", "MNZL,NVDA,ORCL,DV,CTSH,QNRX,AMD,SPRE,HLAL,SPUS,U,AG,SPCX,AAPL,PAAS,GC=F"))
...
with tab_chart:
    ...
    if clean_symbols_list:
        ...
        if selected_sym:
            ...
            # تعديل 4: عرض أخبار السهم
            st.subheader("📰 أخبار ومؤثرات السهم")
            try:
                ticker_news = yf.Ticker(target_clean)
                news_list = ticker_news.news
                if news_list:
                    for n in news_list[:3]:
                        st.write(f"• [{n['title']}]({n['link']})")
                else:
                    st.write("لا توجد أخبار حديثة متاحة.")
            except:
                st.write("تعذر جلب الأخبار.")
...
with tab_watchlist:
    ...
    # تعديل 2: إضافة الفريم فوق الجدول
    watch_calc_frame = st.selectbox(
        "⏱️ اختر فريم مراقبة المحفظة:",
        ["⚡ 5 دقائق", "⏱️ 4 ساعات", "📈 يومي", "💼 شهري"],
        key="watchlist_frame"
    )
    if "5 دقائق" in watch_calc_frame: w_period, w_interval = "5d", "5m"
    elif "4 ساعات" in watch_calc_frame: w_period, w_interval = "60d", "4h"
    elif "يومي" in watch_calc_frame: w_period, w_interval = "2y", "1d"
    else: w_period, w_interval = "10y", "1mo"
    
    st.caption(f"تحديث تلقائي مستمر للفريم: ({watch_calc_frame})")
    ...
    if clean_symbols_list:
        ...
        with st.spinner("جاري تحديث البيانات اللحظية..."):
            for sym in clean_symbols_list:
                ...
                try:
                    sym_df = fetch_clean_data(sym_clean, w_period, w_interval)
